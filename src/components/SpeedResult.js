...
export default function SpeedResult() {
  ...
  const getAverageData = () => {
      ...
      if(openedDSDetails && openedDSDetails.dsDetails.JSONResults){ // if dstructures are not empty
          ...
          const sum = results.reduce((total, value) => total + value.speedms, 0)
          ...
          return {dsname: dstructures[openedDSDetails.dsIndex].dsname, mean: roundedMean, standardDeviation: rounded, speedms: roundedSum}
      }
  ...
  const getAllResultsSummary = () => {
      ...
      if(openedDSDetails && openedDSDetails.dsDetails.JSONResults){ // if dstructures are not empty
          ...
          for(let i = 0; i < results.length; i++){
              ...
              speedms = Math.floor(speedms * 1e6) / 1e6;
  ...
  const getAllRunsNotation = () => {
    ...
    if(openedDSDetails && openedDSDetails.dsDetails.JSONResults){ // if dstructures are not empty
        ...
        for(let i = 0; i < results.length; i++){
            let currentIndex = results[i].currentIndex
            let speednotation = notationToInt(results[i].speednotation)
            let diff = prevNotation ? speednotation - prevNotation : 0 
            let sign = diff >= 0 ? "+" : "-"

            prevNotation = speednotation 

            //formatting
            diff = Math.abs(diff)
            speednotation = intToNotation(speednotation)

            arr.push({currentIndex, speednotation, diff, sign})
        }
    } 
    return arr
}
        ...
        <div className='min-h-[450px] h-full w-full flex px-5 py-4 gap-5'>
           ...
           <div className='bg-gray-50 shadow3 rounded h-full max-h-[400px] overflow-auto flex-1 flex flex-col'>
                ...
                <div className='w-full h-full'>
                    <List>
                        <ListItemButton>
                            <ListItemText primary={"Overall Speed"} secondary={averageData.speedms + "ms"} />
                        </ListItemButton>
                        ...
                        <ListItemButton>
                            ...
                            <ListItemText primary={"Average Speed"} secondary={averageData.mean + "ms"} />
                        </ListItemButton>

                        <Divider />

                        <ListItemButton>
                            <ListItemText primary={"Standard Deviation"} secondary={averageData.standardDeviation + "ms"} />
                        </ListItemButton>
                ...
                <div className='w-full h-full'>
                    <List>
                        {allResults.map((item, index) => (
                            <div key={index}>
                                <ListItemButton>
                                  <ListItemText primary={"Index: " + item.currentIndex} secondary={item.speedms + ` ms`} />
                ...
                <div className='w-full h-full'>
                    <List>
                        {allResultsNotation.map((item, index) => (
                            <div key={index}>
                                <ListItemButton>
                                  <ListItemText primary={"Index: " + item.currentIndex} secondary={item.speednotation} />
